...
public class MainMenuActivity extends BaseActivity implements
		...
		ChangeNameDialog.OnChangeUsernameListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!this.mPrefs.isLoggedIn()) {
			new ChangeNameDialog().show(getFragmentManager(),
					"change-name-dialog");
		} else {
			Toast.makeText(
					this,
					"Welcome "
							+ mPrefs.getUserDetails().get(SessionManager.USER),
	...
	private void showPopupMenu(View v) {
		...
		popup.getMenu()
				.findItem(R.id.menu_change_mode)
				.setTitle(
						"Change to "
								+ (mPrefs.isDiscordMode() ? "Harmony"
										: "Discord") + " mode");
		popup.show();
	}
	...
	public boolean onMenuItemClick(MenuItem item) {
		...
		case R.id.menu_change_mode:
			mPrefs.setDiscordMode(!mPrefs.isDiscordMode());
			Intent restartIntent = new Intent(this,MainMenuActivity.class);
