	...
	public void handleMessage(Message msg) {
		...
		case BluetoothChatService.MESSAGE_READ:
			...
			if (!readMessage.isEmpty()) {
				...
				// parse in the json message
				ObjectMapper mapper = JsonUtils.getObjectMapper();

				JsonNode root = null;
				try {
					root = mapper.readTree(readMessage);
				} catch (IOException e) {
				...
				if (root != null) {
					JsonNode bCards = root.get("player_id");
					JsonNode wCards = root.get("card_id");
					// TODO: do interesting things with the data
				}
			}
			break;
